...
using Import_Export_Universel.Properties;
...
namespace Import_Export_Universel
{
	public class ConnexionSQLServer : ConnexionBD
	{
		...
		public override IDataReader ExecuteReader(string sqlQuery, List<DbParameter> listeDesParamètres)
		{
			if (this.dbConnexion != null)
			{
				lock (this.dbConnexion)
				{
					...
					if (true == this.options.ConnexionODBC)
						this.dbCommand  = new OdbcCommand(sqlQuery, (this.dbConnexion as OdbcConnection));
					else
						this.dbCommand  = new SqlCommand(sqlQuery, (this.dbConnexion as SqlConnection));
					...
					this.dataReader = dbCommand.ExecuteReader(CommandBehavior.Default);
				}
			}
		...
		public override object ExecuteScalar(string sqlQuery, List<DbParameter> listeDesParamètres)
		{
			...
			//DbCommand dbCommand=null;
			//GestionnaireDesTâches.Message( "1111" );
			try
			{
				lock (this.dbConnexion)
				{
					...
					if (null != listeDesParamètres)
					{
			//GestionnaireDesTâches.Message( "2222 - liste : " + listeDesParamètres.Count );
						foreach (DbParameter param in listeDesParamètres)
						{
							if (param.Value != null && (param.Value.ToString().Length == 0 || param.Value.ToString().Equals("NULL")))
								param.Value = DBNull.Value;
			...
			//GestionnaireDesTâches.Message( "ajout : " + param.ParameterName + " de valeur : " + param.Value);
						}
					}
