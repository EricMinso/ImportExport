...
namespace Import_Export_CSV
{
    public partial class FormPrincipale : Form
    {
        ...
        private void Deconnexion()
        {
            ...
            gbBD.Enabled = false;
            btExecuterRequete.Enabled = false;
        ...
        private void OuvrirFichierSQL()
        {
            try
            {
                ...
                if (DialogResult.OK == openSqlFileDialog.ShowDialog())
                {
                    ...
                    {
                        tbRequete.AppendText(strChaineLue);
                    }
        ...
        private void EnregistrerTamponDansFichier(string strFichier)
        {
            // Ne devrait pas arriver
            if (TamponCourant == null ||
                TamponCourant.Columns.Count < 1 )
                //TamponCourant.Rows.Count < 1)
            {
                TamponCourant = null;
                ActualiserAffichageSelonTampon();
                return;
        ...
        private void ExecuterRequeteSQL()
        {
            ...
            try
            {
                ...
                //SqlDataReader sqlReader = sqlCommand.ExecuteReader();
                IDataReader reader = GestionnaireBD.Connexion.ExecuteReader(tbRequete.Text);
        ...
        void dgvTampon_DataError(object sender, DataGridViewDataErrorEventArgs e)
        {
            ...
            GestionnaireDesTâches.addException(
        ...
        private void lbListeTables_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                //lbNomTableSélectionnée.Text = strTable;
                ...
                // Liste des colonnes de la table
                // SqlCommand sqlCommand = new SqlCommand("SELECT COLUMN_NAME, DATA_TYPE, IS_NULLABLE, COLUMN_DEFAULT FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME LIKE '" + strTable + "'", sqlConnexion);
                // SqlDataReader sqlReader = sqlCommand.ExecuteReader();
                //List<string> listeColonnes = this.connexionBD.ListeDesColonnes(strTable);

                //DataTable dataTable = new DataTable("Colonnes BD");
                //dataTable.Columns.Add(new DataColumn("Nom de colonne"));
                /*dataTable.Columns.Add(new DataColumn("Type de données"));
                dataTable.Columns.Add(new DataColumn("Null autorisé"));
                dataTable.Columns.Add(new DataColumn("Valeur par défaut"));

                // Remplissage des données
                lbListeChamps.Items.Clear();

                while (sqlReader.Read())
                {
                    DataRow row = dataTable.NewRow(); //new DataRow():
                    row[0] = sqlReader[0].ToString();
                    row[1] = sqlReader[1].ToString();
                    row[2] = sqlReader[2].ToString();
                    row[3] = sqlReader[3].ToString();
        ...
        private void btEnregistrerDansFichier_Click(object sender, EventArgs e)
        {
            ...
            {
                //TaskDialog
                MessageBox.Show("Il me faut un nom pour le fichier", "Divination impossible", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else
                EnregistrerTamponDansFichier(tbRepertoire.Text + "\\" + tbNomFichierExport.Text);
        }
...
/*                "export_" + tabTampons.SelectedTab.Text + "_" +
        ...
        private void btSupprimerConfig_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (null == configASupprimer)
                {
                    ...
                    "Supprimer la configuration " + configASupprimer.NomConfig + " ? ",
                    "Suppression",
        ...
        private void cbModèlePrédéfini_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            tbRequete.AppendText("\r\n" + cbModèlePrédéfini.SelectedItem + "\r\n");
            tbRequete.Focus();
        }
        ...
        private void cbMasquerPanelHaut_CheckedChanged(object sender, EventArgs e)
        {
            gbTables.Visible = cbMasquerPanelHaut.Checked;
            tabCommandes.Visible = cbMasquerPanelHaut.Checked;
            tlpGestionnaireBoutonsTampons.Visible = cbMasquerPanelHaut.Checked;
            splitContainerPrincipal.Panel1Collapsed = !cbMasquerPanelHaut.Checked;
        ...
        {
            if (DialogResult.OK == MessageBox.Show(
                    "Générer un script dans la fenêtre SQL pour effacer toutes les données de la base ? ",
                    ...
                    MessageBoxIcon.Question))
            {
                ...
                foreach (InfoTable table in lbListeTablesVues.Items)
                    tbRequete.Text += "DELETE FROM [" + table.NomTable + "]\r\n";
        ...
        private void btEffacerTable_Click(object sender, EventArgs e)
        {
            if (lbListeTablesVues.SelectedItem != null)
            {
                ...
                if (DialogResult.OK == MessageBox.Show(
                        "Générer un script dans la fenêtre SQL pour effacer les données de la table " + strTable + " ? ",
                        "Confirmez la suppression",
                        MessageBoxButtons.OKCancel,
                        MessageBoxIcon.Question))
                {
